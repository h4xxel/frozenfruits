...
void *snsbbfzOpen(const char *fname) {
	...
	if ((fs = malloc(sizeof(SNSBBFZ_FILE))) == NULL) {
		fprintf(stderr, "sgputil: Error: malloc has failed us\n");
		return NULL;
	}
...
void *snsbbfzGetHeaderData(void *handle, unsigned int n) {
	...
	unsigned int *index = fs->headerdata;
	void *payload;
...
void *snsbbfzGetData(void *handle, unsigned int n, unsigned int *len) {
	...
	index = &index[sizeof(SNSBBFZ_HEADINFO) >> 2];
	fh = (void *) &index[((sizeof(SNSBBFZ_HEADER) + fs->headerpadding) >> 2) * n];
...
void *snsbbfzPrepareNew() {
	SNSBBFZ_FILE *file;
	if ((file = malloc(sizeof(SNSBBFZ_FILE))) == NULL) {
		fprintf(stderr, "sgputil: Error: Malloc has failed us\n");
		return NULL;
	}
...
int snsbbfzAddFile(void *handle, const char *fname, const char *headdata) {
	...
	if (headdata != NULL) {
		if ((fp = fopen(headdata, "r")) == NULL) {
			...
			file->sfiles = realloc(file->sfiles, sizeof(SNSBBFZ_HEADINFO) * fid);
			file->dataheaders--;
			return -1;
		}
...
int snsbbfzWriteOut(void *handle, const char *fname) {
	...
	unsigned int datapos, i, gziplen;
	SNSBBFZ_MH mh;
	...
	offsetCalc = (void *) &hf[1];
	
	for (i = 0; i < hf->dataHeaders; i++) {
		hh = (void *) &offsetCalc[(file->headerpadding+sizeof(SNSBBFZ_HEADER))*i];
...
/*		if (file->sfiles[i].flags & COMPRESSION_LZMA)
			headerData = xzCompress(tempData, file->sfiles[i].len, &datapos);
		else if (file->sfiles[i].flags & COMPRESSION_BZIP)
			headerData = bz2Compress(tempData, file->sfiles[i].len, &datapos);*/
		/*else if (file->sfiles[i].flags & COMPRESSION_GZIP)*/
			headerData = zCompress(tempData, file->sfiles[i].len, &datapos);
		/*else
			headerData = NULL;*/
		free(tempData);
